...
<template>
  <div class="px-8 pt-5 bg-white dark:bg-gray-900 md:flex md:justify-center">
    <ol class="relative border-s border-gray-200 dark:border-gray-700">
      <li class="ms-6" :class="{ 'mb-10': index !== list.length - 1 }" v-for="(item, index) in list" :key="index">
        <span
        ...
        <h3 class="flex items-center mb-1 text-lg font-semibold text-gray-900 dark:text-white">{{ item.title }}<span
            v-if="index === 0"
            class="bg-blue-100 text-blue-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300 ms-3">最新!~</span>
        </h3>
        <time class="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">{{ item.time }}</time>
        <p class="mb-4 text-base font-normal text-gray-500 dark:text-gray-400">{{ item.desc }}</p>
